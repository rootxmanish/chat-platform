...
const Login = () => {
  ...
  return (
    <div className="min-h-screen bg-void-900 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-iris-600/10 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="w-full max-w-sm relative z-10">
        ...
        <div className="border-gradient rounded-2xl p-7 bg-surface">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full relative overflow-hidden rounded-xl py-3 text-sm font-semibold text-white bg-gradient-to-r from-iris-500 to-iris-600 hover:from-iris-400 hover:to-iris-500 disabled:opacity-60 disabled:cursor-not-allowed transition-all duration-200 shadow-glow-sm hover:shadow-glow-iris mt-2"
            >
              {loading ? (
        ...
        <p className="text-center text-sm text-gray-600 mt-6">
          No account?{" "}
          <Link to="/register" className="text-iris-400 hover:text-iris-300 font-medium transition-colors">
            Create one
          </Link>
        </p>
      </div>
